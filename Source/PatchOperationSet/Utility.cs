...
public static class Utility
{
	...
	public static bool ContainsOnlyText(this object[] valueChildNodes)
		=> Array.Find(valueChildNodes, static node => node is XmlText) != null
			&& Array.TrueForAll(valueChildNodes, static node => node is not (XmlElement or XmlAttribute));

	public static string GetInnerText(this object[] valueChildNodes)
		=> valueChildNodes.Length == 1
			? ((XmlNode)valueChildNodes[0]).InnerText
			: string.Concat(valueChildNodes.Select(static node => ((XmlNode)node).InnerText));
